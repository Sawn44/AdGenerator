...
import { useHusseResources } from '../../hooks/useHusseResources';
import type { HusseResource } from '../../hooks/useHusseResources';
...
export const HusseResourceModal: React.FC<HusseResourceModalProps> = ({
  ...
  const { resources, loading, error, fetchResources } = useHusseResources();

  useEffect(() => {
    if (isOpen && resources.length === 0) {
      fetchResources();
    }
  }, [isOpen, resources.length, fetchResources]);
      ...
      <div className="bg-gray-800 rounded-lg shadow-xl w-full max-w-3xl max-h-[80vh] flex flex-col">
        <div className="flex items-center justify-between p-4 border-b border-gray-700">
          <h2 className="text-lg font-semibold text-gray-100">
            Wybierz {filter === 'logo' ? 'logo' : 'packshot'} z Husse
          </h2>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto p-4">
          ...
          {!loading && !error && filteredResources.length === 0 && (
            <div className="flex items-center justify-center py-8">
              <div className="text-gray-400">Brak dostępnych zasobów</div>
            </div>
          )}
